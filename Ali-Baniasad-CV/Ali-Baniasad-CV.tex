...
\vspace{-6pt}
    \begin{itemize}
		\item Proposed s{LSTM–MLP} that performs in-motion using only IMU + SINS data when GPS is unavailable.  
...
\begin{itemize}  \itemsep 1pt % Slightly reduced spacing for sub-items
    ...
    \item  {Real-time} visualization and {interactive} adjustments for aircraft performance {evaluation}.
...
\begin{itemize}  \itemsep 1pt % Slightly reduced spacing for sub-items
    ...
    \item Developed an {automated design cycle} that allows for seamless updates across all stages.
\end{itemize}
...
\begin{itemize}
  \setlength\itemsep{-2pt}       % no extra package needed
  \vspace{-4pt}
